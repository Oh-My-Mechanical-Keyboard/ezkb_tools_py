...
class PlateGenerator(object):
    ...
    #init
    def __init__(self, arg_ct, arg_cr, arg_st, arg_sr, arg_at, arg_ar, arg_uw, arg_uh, arg_key_north, arg_key_rgb, arg_gen_type, arg_db):
        ...
        # Key direction
        self.key_in_north = arg_key_north

        # Has key rgb
        self.key_rgb = arg_key_rgb

        # gen type
        self.gen_type = arg_gen_type
    ...
    # Draw circul segment rotated with respect to an anchor
    def draw_rotated_circle(self, x, y, anchor_x, anchor_y, radius, angle):
        coords = self.rotate_point_around_anchor(x, y, anchor_x, anchor_y, angle)
        self.modelspace.add_circle((coords[0], coords[1]), radius)
    ...
    # Draw switch hole
    def draw_switch_hole(self, x, y, angle):
        ...
        if (self.cutout_type in standard_cutout_types):
            self.draw_rotated_circle(x, y, anchor_x, anchor_y, Decimal('2'), angle)
            self.draw_rotated_circle(x-Decimal('5.08'), y, anchor_x, anchor_y, Decimal('0.9'), angle)
            self.draw_rotated_circle(x+Decimal('5.08'), y, anchor_x, anchor_y, Decimal('0.9'), angle)
            if (self.key_in_north):
                self.draw_rotated_circle(x-Decimal('3.81'), y-Decimal('2.54'), anchor_x, anchor_y, Decimal('1.5'), angle)
                self.draw_rotated_circle(x+Decimal('2.54'), y-Decimal('5.08'), anchor_x, anchor_y, Decimal('1.5'), angle)			
            else:
                self.draw_rotated_circle(x-Decimal('3.81'), y+Decimal('2.54'), anchor_x, anchor_y, Decimal('1.5'), angle)
                self.draw_rotated_circle(x+Decimal('2.54'), y+Decimal('5.08'), anchor_x, anchor_y, Decimal('1.5'), angle)
            ...
            if (self.key_rgb):
                ...
                self.draw_rotated_line(rgb_c_x - Decimal('2.5'), rgb_c_y + Decimal('2.5'), rgb_c_x - Decimal('2.5'), rgb_c_y - Decimal('2.5'), anchor_x, anchor_y, angle)
                self.draw_rotated_line(rgb_c_x + Decimal('2.5'), rgb_c_y + Decimal('2.5'), rgb_c_x + Decimal('2.5'), rgb_c_y - Decimal('2.5'), anchor_x, anchor_y, angle)
                self.draw_rotated_line(rgb_c_x - Decimal('2.5'), rgb_c_y + Decimal('2.5'), rgb_c_x + Decimal('2.5'), rgb_c_y + Decimal('2.5'), anchor_x, anchor_y, angle)
                self.draw_rotated_line(rgb_c_x - Decimal('2.5'), rgb_c_y - Decimal('2.5'), rgb_c_x + Decimal('2.5'), rgb_c_y - Decimal('2.5'), anchor_x, anchor_y, angle)

    # Draw hotswap key base
    def draw_hotswap_base_hole(self, x, y, angle):
    ...
    def render_switch(self, switch):
        ...
        if (self.gen_type == 'up'):
            #Draw main switch hole
            self.draw_switch_hole(switch.mm_center[0], switch.mm_center[1], switch.angle + switch.cutout_angle)

        if (self.gen_type == 'p' or self.gen_type == 'up'):
...
if __name__ == "__main__":
    ...
    args.unit_width, args.unit_height, args.key_north, args.key_rgb, args.gen_type, args.debug_log)
